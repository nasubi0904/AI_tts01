...
class TalkPipeline:
    def __init__(self, system_prompt: str = "", reporter: Reporter|None=None):
        ...
        if OLLAMA_STREAM_SENTENCE_LIMIT:
            limits.append(f"文数≦{OLLAMA_STREAM_SENTENCE_LIMIT}")
        if OLLAMA_STREAM_CHAR_LIMIT:
            limits.append(f"文字数≦{OLLAMA_STREAM_CHAR_LIMIT}")
        if OLLAMA_STREAM_TIMEOUT:
            limits.append(f"経過時間≦{OLLAMA_STREAM_TIMEOUT:g}秒")
        if limits:
            log(
                "INFO",
                "LLM応答制御: " + ", ".join(limits) + " (num_predict 以外の制御を適用中)",
            )
