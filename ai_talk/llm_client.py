...
from .config import (
    ...
    OLLAMA_STREAM_SENTENCE_LIMIT,
    OLLAMA_STREAM_CHAR_LIMIT,
...
def _apply_text_limits(text: str) -> tuple[str, list[str]]:
    """文数・文字数の上限を適用し、必要なら理由を返す。"""
    ...
    if OLLAMA_STREAM_SENTENCE_LIMIT:
        ...
        while i + 1 < len(parts):
            ...
            count += 1
            if count >= OLLAMA_STREAM_SENTENCE_LIMIT:
                break
        if count >= OLLAMA_STREAM_SENTENCE_LIMIT:
            trimmed = "".join(collected)
            reasons.append(
                f"LLM応答を文数上限 {OLLAMA_STREAM_SENTENCE_LIMIT} で打ち切りました。"
            )

    if OLLAMA_STREAM_CHAR_LIMIT and len(trimmed) > OLLAMA_STREAM_CHAR_LIMIT:
        clipped = trimmed[:OLLAMA_STREAM_CHAR_LIMIT].rstrip()
        if clipped:
            trimmed = clipped
        else:
            trimmed = ""
        reasons.append(
            f"LLM応答を文字数上限 {OLLAMA_STREAM_CHAR_LIMIT} で打ち切りました。"
        )
    ...
    for idx, (url, force_chat) in enumerate(candidates):
        ...
        with _session_post(
            ...
            try:
                ...
                raise
            data = response.json()
            text, reasons = _apply_text_limits(_extract_response_text(data))
...
def stream(prompt: str, system: str = "") -> Iterable[str]:
    ...
    for idx, (url, force_chat) in enumerate(candidates):
        ...
        with response:
            buf = ""
            sentence_limit = OLLAMA_STREAM_SENTENCE_LIMIT or 0
            char_limit = OLLAMA_STREAM_CHAR_LIMIT or 0
            timeout_limit = OLLAMA_STREAM_TIMEOUT or 0.0
            sentence_count = 0
            char_count = 0
            start_time = time.monotonic()

            def _apply_char_limit_local(text: str) -> tuple[str, bool]:
                """文字数上限を適用し、切り詰めたかどうかを返す。"""
                ...
                if not char_limit:
                    char_count += len(text)
                    return text, False
                remain = char_limit - char_count
                if remain <= 0:
                    return "", True
                if len(text) <= remain:
                    char_count += len(text)
                    return text, False
                clipped = text[:remain].rstrip()
                char_count += len(clipped)
                return clipped, True

            def _after_yield(truncated: bool = False) -> bool:
                """各種閾値を満たしたらログを出して終了する。"""

                if truncated and char_limit:
                    log("INFO", f"LLM応答を文字数上限 {char_limit} で打ち切りました。")
                    return True
                if sentence_limit and sentence_count >= sentence_limit:
                    log("INFO", f"LLM応答を文数上限 {sentence_limit} で打ち切りました。")
                    return True
                if char_limit and char_count >= char_limit:
                    log("INFO", f"LLM応答を文字数上限 {char_limit} で打ち切りました。")
                    return True
                if timeout_limit > 0 and (time.monotonic() - start_time) >= timeout_limit:
                    log("INFO", f"LLM応答を経過時間 {timeout_limit:g} 秒で打ち切りました。")
                    return True
                return False
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                if chunk:
                    ...
                    for i in range(0, len(parts) - 1, 2):
                        ...
                        if not sent:
                            continue
                        sent, truncated = _apply_char_limit_local(sent)
                        if not sent:
                            continue
                        sentence_count += 1
                        yield sent
                        if _after_yield(truncated):
                            return
                ...
                if timeout_limit > 0 and (time.monotonic() - start_time) >= timeout_limit:
                    leftover = buf.strip()
                    if leftover:
                        leftover, truncated_leftover = _apply_char_limit_local(leftover)
                        if leftover:
                            sentence_count += 1
                            yield leftover
                            if truncated_leftover and char_limit:
                                log(
                                    "INFO",
                                    f"LLM応答を文字数上限 {char_limit} で打ち切りました。",
                                )
                            elif sentence_limit and sentence_count >= sentence_limit:
                                log(
                                    "INFO",
                                    f"LLM応答を文数上限 {sentence_limit} で打ち切りました。",
                                )
                            elif char_limit and char_count >= char_limit:
                                log(
                                    "INFO",
                                    f"LLM応答を文字数上限 {char_limit} で打ち切りました。",
                                )
                ...
                if obj.get("done"):
                    ...
                    if leftover:
                        ...
                        if leftover:
                            sentence_count += 1
                            yield leftover
                            if _after_yield(truncated_leftover):
                                return
                    break
            return
